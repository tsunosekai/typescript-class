...
→ デスクトップの場所がわからない場合は `cd ~/Desktop` を試してください
...
```
→ インターネット接続を確認してください
→ 会社や学校のネットワークだと、プロキシ設定が必要な場合があります
→ 別のWi-Fiネットワークで試してみてください
→ 時間をおいてから（数分〜数時間後）再度試してください
...
```
→ フォルダの書き込み権限がない可能性があります
→ デスクトップ以外の場所（Documents フォルダなど）で試してみてください
→ 管理者権限が必要な場合は、管理者に相談してください

**エラー例④：すでにフォルダが存在する**
```
EEXIST: file already exists
```
→ `react-shindan` フォルダが既に存在しています
→ 既存のフォルダを削除するか、別の名前でプロジェクトを作成してください
→ 例：`npm create vite@latest react-shindan2 -- --template react-ts`

**エラー例⑤：Node.jsのバージョンが古い**
```
npm WARN npm does not support Node.js
```
→ Node.jsを最新のLTS版に更新してください
