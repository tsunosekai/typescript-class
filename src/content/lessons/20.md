...
<details>
<summary>🔧 useStateがうまく動かないときは</summary>

**画面が更新されない**
- `count = count + 1` ではなく `setCount(count + 1)` を使っていますか？
- 配列やオブジェクトの場合、新しく作る必要があります: `setItems([...items, "新しい"])`

**useStateでエラーが出る**
- `useState is not defined` → `import { useState } from "react";` を追加
- `Hooks can only be called inside...` → `useState` はコンポーネント関数の中で使う
- `Too many re-renders` → `setXxx` をレンダリング中に直接呼んでいませんか？

**ボタンが反応しない**
- `onClick={handleClick()}` ではなく `onClick={handleClick}` （カッコなし）
- または `onClick={() => handleClick()}` と書く

**入力フォームが動かない**
- `value` と `onChange` の両方が必要です
...
> **`...items` とは？（スプレッド構文）**
> `...` は配列やオブジェクトの中身を「展開」する書き方です。
> ```tsx
> const items = ["A", "B", "C"];
> [...items, "D"]  // → ["A", "B", "C", "D"]
> ```
> `[...items, "D"]` は「itemsの中身を展開して、最後にDを追加した**新しい配列**」を作ります。
