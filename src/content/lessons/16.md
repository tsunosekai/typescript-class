...
title: "【作品③】クイズゲームを作ろう"
lesson: 16
goal: "オブジェクトと配列を使ってクイズゲームを完成させる"
time: "約50分"
checklist:
  - "クイズデータをオブジェクトの配列で管理できた"
  - "問題を順番に表示できた"
  - "スコアを計算できた"
  - "結果画面が表示された"
---
...
![screenshot: クイズゲームの完成イメージ](./images/quiz-complete.svg)
...
**症状①：画面が真っ白**
→ コンパイルしましたか？（`npx tsc quiz.ts`）
→ HTMLが `quiz.js`（.ts ではなく .js）を読み込んでいるか確認：
```html
<script src="quiz.js"></script>  <!-- quiz.ts ではない -->
```
→ Live Serverで開いていますか？（「Go Live」をクリック）
...
**症状③：選択肢のボタンが表示されない**
→ `choicesElement` が正しく取得できているか確認
→ HTMLの `id="choices"` と `querySelector("#choices")` が一致しているか確認

---

### コンパイルエラーが出る

**エラー例①：Property does not exist on type 'Element'**
```
Property 'style' does not exist on type 'Element'
```
→ `as HTMLElement` を付けていない可能性があります
```typescript
// ❌ エラーになる
...
**エラー例②：Object is possibly 'null'**
```
Object is possibly 'null'
```
→ 要素が見つからない可能性があるというエラーです
→ `if` で存在確認するか、`?.`（オプショナルチェイニング）を使います
```typescript
// 方法1: if で確認
if (element) {
...
→ `addEventListener` が正しく書かれているか確認
```typescript
// ✅ 正しい書き方
nextButton?.addEventListener("click", nextQuestion);
```
...
**原因②：要素が見つかっていない**
→ `console.log(nextButton)` で確認。`null` なら見つかっていません
→ HTMLの `id` と `querySelector` の `#id` が一致しているか確認

**原因③：`isAnswered` フラグが true のまま**
→ `selectAnswer` 関数の最初で `if (isAnswered) return;` がありますか？
→ リセット時に `isAnswered = false;` を忘れていないか確認

---

### スコアが正しく計算されない

**原因①：`correctIndex` の値が間違っている**
→ 配列のインデックスは 0 から始まります
```typescript
// "東京" は配列の 2番目（インデックスは2）
choices: ["大阪", "京都", "東京", "名古屋"]
//          0       1       2        3
correctIndex: 2  // "東京" を正解にする
```

**原因②：`score` 変数がリセットされていない**
→ リトライ時に `score = 0;` を忘れていないか確認

---

### 結果画面に遷移しない

**原因①：`currentQuizIndex` の比較が間違っている**
```typescript
// ✅ 正しい
if (currentQuizIndex < quizData.length) {
    showQuestion();
...
→ F12でコンソールを開く
→ 赤いエラーメッセージを読む
→ 行番号をクリックすると、エラーの場所がわかる

3. **一度に全部書かず、少しずつ確認する**
→ まず問題表示だけ動かす → 次に回答チェック → 最後に結果画面
