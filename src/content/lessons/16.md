...
<details>
<summary>🔧 クイズが動かないときは</summary>

**画面が真っ白**
→ コンパイルしましたか？（`npx tsc quiz.ts`）
→ HTMLが `.js` ファイルを読み込んでいるか確認（`.ts` ではない）
→ Live Serverで開いていますか？

**ボタンが反応しない**
→ `addEventListener` が正しく設定されているか確認
→ `console.log(nextButton)` で要素が取得できているか確認（`null` なら id が間違っている）

**コンパイルエラー**
→ `as HTMLElement` を忘れていませんか？
→ `if (element)` で null チェックしていますか？

**スコアがおかしい**
→ `correctIndex` の値を確認（配列は0から始まる）
→ リトライ時に `score = 0` でリセットしていますか？
