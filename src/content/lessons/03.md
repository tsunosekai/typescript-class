...
👉 <a href="https://nodejs.org/" target="_blank" rel="noopener noreferrer"><strong>https://nodejs.org/</strong></a>
...
<details>
<summary>🔧 「npm install」でエラーが出るときは</summary>
...
**対処法：**
1. **インターネット接続**を確認してください
2. 時間がかかることもあるので、**5分くらい待って**みましょう
3. `Ctrl + C` で止めてから、もう一度 `npm install` を実行
...
<details>
<summary>🔧 「npx ts-node hello.ts」でエラーが出るときは</summary>
...
| 方法 | コマンド | 使う場面 | 特徴 |
|------|---------|---------|------|
| **ts-node** | `npx ts-node ファイル.ts` | ターミナルで動かす（第3〜12回） | 手軽。ブラウザでは使えない |
| **tsc** | `npx tsc ファイル.ts` | ブラウザで動かす（第13〜16回） | JSファイルに変換。毎回手動でコンパイル |
| **Vite** | `npm run dev` | Reactで開発（第17回〜） | 自動でコンパイル＆更新。一番便利 |

**なぜ違うの？**
- **ターミナル**で動かすときは、Node.jsが直接実行するので `ts-node` が使える
- **ブラウザ**で動かすときは、ブラウザがTypeScriptを理解できないので、JavaScriptに変換（`tsc`）が必要
- **React**では、Viteが裏で自動的に変換してくれるので、何も意識しなくてOK
