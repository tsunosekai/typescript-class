...
time: "約30分"
checklist:
  - "let と const の違いがわかった"
  - "string / number / boolean の型がわかった"
  - "型を付けて変数を宣言できた"
---
...
<details>
<summary>🔧 プログラムが実行できないときは</summary>

**エラー例①：ファイルが見つからない**
```
Cannot find module '/path/to/variables.ts'
```
→ ファイル名を確認してください（`variables.ts`）
...
<details>
<summary>🔧 型エラーが出るときは</summary>

**エラー例①：型が合わない**
```
Type 'string' is not assignable to type 'number'
```
→ 数値を入れるべき場所に文字列を入れています
```typescript
// ❌ エラーになる
let age: number = "16";  // "16" は文字列

// ✅ 正しい
let age: number = 16;    // 16 は数値
```

**エラー例②：constの値を変更しようとした**
```
Cannot assign to 'PI' because it is a constant
```
→ `const` で宣言した変数は変更できません。変更したいなら `let` を使ってください。

**エラー例③：変数が宣言されていない**
```
Cannot find name 'myName'
```
→ 変数名のスペルミス、または宣言していない可能性があります

**よくある間違い**

- **文字列の引用符忘れ**: `let name = 山田太郎;` → `let name = "山田太郎";`
- **大文字・小文字**: `String` ではなく `string`（小文字）を使う
