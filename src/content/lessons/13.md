...
### Live Serverが起動しない・動作しない

**症状①：「Go Live」ボタンが見つからない**
→ VSCodeの右下を確認してください
→ 見つからない場合は、Live Server拡張機能がインストールされていない可能性があります
→ 拡張機能（四角が4つのアイコン）→「Live Server」で検索 → インストール

**症状②：「Go Live」をクリックしても何も起きない**
→ HTMLファイルを開いた状態でクリックしていますか？
→ フォルダではなく、HTMLファイルを直接開いてから試してください
→ VSCodeを再起動してみてください

**症状③：ブラウザが開くが「このサイトにアクセスできません」と表示される**
→ ポート（5500）が他のプログラムに使われている可能性があります
→ パソコンを再起動して、もう一度試してください
→ ファイアウォールがブロックしている可能性があります（管理者に相談）

**症状④：HTMLファイルが表示されるが、JSが動かない**
→ HTMLが正しいJSファイルを読み込んでいるか確認：
```html
<script src="dom-practice.js"></script>  <!-- .ts ではなく .js -->
```
→ TypeScriptをコンパイルしたか確認（`npx tsc ファイル名.ts`）

**症状⑤：ブラウザのURLが `file://...` になっている**
→ Live Server経由ではなく、HTMLファイルを直接開いてしまっています
→ VSCodeで「Go Live」をクリックして開き直してください
→ 正しいURLは `http://127.0.0.1:5500/...` のような形式です
...
**症状①：F12を押しても何も起きない**
→ ブラウザ上で押していますか？（VSCodeではなく）
→ メニューから開く方法：
  - Chrome: 右上の「︙」→「その他のツール」→「デベロッパーツール」
  - Edge: 右上の「...」→「その他のツール」→「開発者ツール」
  - Firefox: 右上の「≡」→「その他のツール」→「ウェブ開発者ツール」

**症状②：Consoleタブが見つからない**
→ 開発者ツールの上部にタブが並んでいます
→ 「Console」または「コンソール」というタブをクリックしてください
→ タブが見えない場合は、開発者ツールの幅を広げてみてください
