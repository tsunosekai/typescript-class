...
> |--|-------------------|---------------------|
> | **実行場所** | ターミナル | ブラウザ |
> | **コマンド** | `node ファイル名.ts` | `npx tsc ファイル名.ts` |
...
> （Node.js 24 ではターミナルで `node` コマンドが直接 TypeScript を実行できますが、ブラウザでは変換が必要です）
...
> **なぜ `node` じゃなくて `tsc` なの？**
> - `node`：ターミナルで動かすとき（Node.js 24 が直接実行）
> - `tsc`：ブラウザで動かすとき（JSファイルに変換が必要）
>
...
<details>
<summary>🔧 コンパイルでエラーが出るときは</summary>
...
<details>
<summary>🔧 ブラウザで表示されない・動かないときは</summary>

**Live Serverの問題**

- 「Go Live」ボタンが見つからない → 拡張機能「Live Server」をインストール
- クリックしても開かない → HTMLファイルを開いた状態でクリック
- URLが `file://...` → Live Server経由で開いてください（正しくは `http://127.0.0.1:5500/...`）

**コンソールの問題**

- F12で開けない → ブラウザ上で押していますか？メニューから「開発者ツール」を選択
- Consoleタブがない → 開発者ツールの幅を広げてみてください

**変更が反映されない**

1. TypeScriptをコンパイルしたか？（`npx tsc dom-practice.ts`）
2. ファイルを保存したか？（`Ctrl + S`）
3. ブラウザを更新したか？（F5キー）
4. HTMLが `.js` ファイルを読み込んでいるか？（`.ts` ではなく）
